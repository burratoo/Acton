...
package body Oak.Processor_Support_Package.Task_Support is
   ...
   procedure Context_Switch_To_Task (Task_Return_State : out Task_Requested_State)
   is
   begin
      --  A context switch will clobber registers r14 - r16, as tasks will return their
      --  state via these registers.
      Asm ("sc"                    & ASCII.LF & ASCII.HT &     -- Switch to Task
           "mr     %0, r14"        & ASCII.LF & ASCII.HT &
           "mr     %1, r15"        & ASCII.LF & ASCII.HT &
           "mr     %2, r16",
        Outputs => (Oak.Oak_Task.Task_State'Asm_Output ("=r", Task_Return_State.State),
                    Integer'Asm_Output ("=r", Task_Return_State.Parameter (0)),
                    Integer'Asm_Output ("=r", Task_Return_State.Parameter (1))),
           Volatile => True);
   ...
   begin
      Asm ("stwu   r1, -24(r1)"     & ASCII.LF & ASCII.HT &
           ...
           "sc",               -- Context switch to kernel.
        Inputs => (Oak.Oak_Task.Task_State'Asm_Input ("r", Resulting_Task_State.State),
   ...
   procedure Set_Oak_Wake_Up_Timer
     (Wake_Up_At : in Ada.Real_Time.Time)
   is
      use Ada.Real_Time;
   ...
   begin
      ...
      Asm ("mtdec %L0",   -- Load Wake Up Time into decrementer register
   ...
   procedure Sleep_Kernel is
      Exit_Condition : Integer := 0; --  Should change this into a Boolean-like type.
   begin
      --  Set Decremeter Interrupt to jump to the other side of the infinite loop
