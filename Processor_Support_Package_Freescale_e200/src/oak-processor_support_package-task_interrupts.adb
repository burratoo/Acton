...
package body Oak.Processor_Support_Package.Task_Interrupts is
   procedure Initialise_Task_Enviroment is
      subtype HID0_Type is
        ISA.Power.e200.z6.HID.Hardware_Implementation_Dependent_Register_0_Type;
      HID0 : HID0_Type;
   begin
      Asm
   ...
   begin
      ...
      Asm ("stwu   r1, -136(r1)"    & ASCII.LF & ASCII.HT & -- Allocate stack space
           "stwcx. r1,  r0, r1"      & ASCII.LF & ASCII.HT & -- Clear memory reservation
           ...
           "mfxer    r26"           & ASCII.LF & ASCII.HT & -- Next instruction address
      ...
      Asm ("mr    r1,  %0"          & ASCII.LF & ASCII.HT & -- load task stack pointer.
           ...
           "mtsrr0    r31"          & ASCII.LF & ASCII.HT & -- Next instruction address
   ...
   begin
      ...
      Asm ("stwu   r1, -224(r1)"    & ASCII.LF & ASCII.HT & -- Allocate stack space
           "stwcx. r1,  r0, r1"     & ASCII.LF & ASCII.HT & -- Clear memory reservation
           "msync"                  & ASCII.LF & ASCII.HT &
           "evstdd r0,  216(r1)"    & ASCII.LF & ASCII.HT & -- Note that we only allocate
           "evstdd r2,  208(r1)"    & ASCII.LF & ASCII.HT & -- space for the GPR at this
           "evstdd r3,  200(r1)"    & ASCII.LF & ASCII.HT & -- access 248 bytes through
           "evstdd r4,  192(r1)"    & ASCII.LF & ASCII.HT & -- the offset referencing
           "evstdd r5,  184(r1)"    & ASCII.LF & ASCII.HT &
           ...
           --  and ______________ and are then set to the return state of the the task.
           ...
           "mfspefscr   r25"        & ASCII.LF & ASCII.HT & -- Next instruction address
           "mfxer       r26"        & ASCII.LF & ASCII.HT &
           "mflr        r27"        & ASCII.LF & ASCII.HT &
      ...
      Asm ("mfsprg0   r1"           & ASCII.LF & ASCII.HT &  -- load kernel stack pointer
   ...
   procedure Decrementer_Interrupt is
      State : constant Task_Requested_State := (Type_State => Runnable,
   ...
   begin
      Asm ("stwu   r1, -24(r1)"     & ASCII.LF & ASCII.HT &
           ...
           "li     r16, 0"          & ASCII.LF & ASCII.HT,
        Inputs => (Oak.Oak_Task.Task_State'Asm_Input ("I", State.State)),
        Volatile => True);
      E200_Context_Switch_To_Kernel;
