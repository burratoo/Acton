...
package body Oak.Core is
   ...
   --------------------------
   -- Perform_Quick_Switch --
   --------------------------

   procedure Perform_Quick_Switch (Message : in out Oak_Message) is
      pragma Unreferenced (Message);
      --  Note that Message is actually used since a context switch occurs
      --  between the start and end of this procedure.
   begin
      Quick_Switch;
   end Perform_Quick_Switch;
   ...
   procedure Run_Loop is
      Dummy_Message : Oak_Message :=
                        (Message_Type => No_Message, L => 0);
   begin
      Run_Oak (Reason_For_Run => First_Run,
               Message        => Dummy_Message);

      loop
         if Has_Timer_Fired (Current_Timer (This_Oak_Kernel)) then
            Run_Oak (Reason_For_Run => Timer,
                     Message        => Dummy_Message);
         else
   ...
   is
      ...
      Message_Is_Bad : constant Oak_Message :=
                         (Message_Type => Invalid_Message, L => 0);
      --  An invalid message.
   ...
   begin
               ...
               when Attach_Interrupt_Handlers =>
                  ...
                  if Current_Agent in Task_Id then
                     Interrupts.Attach_Handlers
                       (Handlers          => Message.Attach_Handlers,
                        Handler_PO        => Message.Attach_Handler_PO,
      ...
      --  Check to see if any interrupt agents are active and have a priority
      --  equal to and above the agent selected above.

      Handle_Active_Interrupts : declare
         Interrupt_Agent : constant Interrupt_Id :=
                             Find_Top_Active_Interrupt (My_Kernel_Id);
      begin
         --  Select the interrupt agent if it has a priority equal to or higher
         --  than the agent selected above. Note that it does not matter if
         --  either Next_Agent or Interrut_Agent is No_Agent as it maps to the
         --  sleep agent that has a value of Priority'First.
      ...
      --  correspond to the current agent's priority due to the correction that
      --  follows).
      ...
      --  Correct Next Agent. Needed to cover the case where a protected agent
      --  has been selected which cannot be executed directly and for an
      --  activator task which is activating child tasks.
