...
package Interfaces.C is
   ...
   --  ??? Paramaters that should be defined elsewhere
   ...
   type long  is range -(2 ** (Long_Bits - Integer'(1)))
     .. +(2 ** (Long_Bits - Integer'(1))) - 1;
   ...
   type ptrdiff_t is
     range -(2 ** (Ptr_Bits - Integer'(1))) ..
           +(2 ** (Ptr_Bits - Integer'(1)) - 1);

   type size_t is mod 2 ** Ptr_Bits;
